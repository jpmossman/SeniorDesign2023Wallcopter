...
\begin{document}
    ...
    \newpage
    \section{Conclusion}
        The Wallcopter project was mildly successfuly in achieving its objectives of creating a drone attachment that enables inspections in difficulty-to-reach locations such as high-rise buildings and undersides of bridges.
        The developed prototype is able to move along ceilings and has a limited set of automated controls.
        An upwards facing camera from the ESP32 as well as the built-in front facing camera of the Tello stream their video to a user-interface that is able to (admittedly, very sensitivly) detect cracks with a custom model that we have trained on a dataset of over 10,000 crack images.
        Both the physical attachment and the user-software are highly extensible, with the primary limitations being weight and processing speed, respectively.
        Future developments could involve using a larger drone that is more capable of withstanding small, unavoidable drafts in open air that the Tello cannot handle, as well as being able to carry a much larger payload.
        As a part of this larger payload we could include more sensors, such as higher quality cameras, thermal cameras, or even ground penetrating radar to perform inspections that can find faults hidden beyond the surface.
        Using a custom built drone would significantly drive up the cost well beyond our initial budget, but would enable us to directly interface with and modify the flight controller, providing a much finer degree of control and the ability to significantly reduce the complexity of our current networking solution to provide a smoother user experience.
        Our Wallcopter project provides an innovative and useful tool for inspections in hard-to-reach areas and has potential to be a commercially viable product.
